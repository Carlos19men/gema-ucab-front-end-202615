...
const ResumenContent = () => {
  ...
  const defaultView =
    searchParams.get("view") === "semanal" ? "semanal" : "mensual";

  //Vista Actual (Mensual o Semanal) por defecto viene del parametro o es mensual
  const [vistaActual, setVistaActual] = useState<"mensual" | "semanal">(
    defaultView,
  );
  ...
  const {
    ...
    isLoading,
    isError,
  } = useCalendario(apiDateParams, vistaActual as "mensual" | "semanal");
  ...
  const filteredTasks = allTasks.filter((task) => {
    ...
    if (
      filtroActivo === "en-ejecucion" &&
      (statusNormalizado.includes("ejecucion") ||
        statusNormalizado.includes("ejecución") ||
        statusNormalizado.includes("ejecutando"))
    ) {
      return true;
    }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `resumen_${vistaActual}_${apiDateParams}.pdf`,
      );
  ...
  return (
    <div className="p-6 max-w-7.5xl">
      <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center gap-4 mb-6">
        <div className="items-center gap-1">
          <h1 className="text-2xl font-bold">
            Resumen de Mantenimientos e Inspecciones
          </h1>
      ...
      <div className="w-full flex justify-end items-center gap-4 mb-4">
        {/* Navegación de Meses */}
        <DateNavigator
          label={vistaActual === "mensual" ? "Mes" : "Semana"}
          onPrev={handlePrev}
          onNext={handleNext}
        ></DateNavigator>
        ...
        <div className="flex flex-col gap-2">
          {filteredTasks.length > 0 ? (
            ...
            <div className="text-center py-10 text-gray-400">
              No hay tareas con el estado "
              {opcionesFiltro.find((o) => o.id === filtroActivo)?.label}"
            </div>
          )}
        </div>
      </div>
    ...
    <Suspense
      fallback={
        <div className="p-6 flex justify-center items-center">
          <Loader2 className="animate-spin h-8 w-8 text-gema-green" />
        </div>
      }
    >
      <ResumenContent />
